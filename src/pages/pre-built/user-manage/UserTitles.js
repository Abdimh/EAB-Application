...
const UserTitles = () => {
  ...
  const onFormSubmit = async (submitData) => {
    ...
    try {
      const RoleRes = await instanceAxios.post("Titles", {
        Title: name,
      });
      ...
      if (data.title) CustomToast("Successfully Added", false, "success");
    } catch (error) {
      // 401, 403, 400
  ...
  const onEditSubmit = async (submitData) => {
    ...
    try {
      const RoleRes = await instanceAxios.put("Titles", {
        id: submitData.id,
        Title: name,
      });
  ...
  const suspendUser = async (id) => {
    try {
      const { data } = await instanceAxios.delete(`Titles/${id}`);
      ...
      CustomToast("Successfully Deleted", false, "error");
    } catch (error) {
      // 401, 403, 400
      if (isAxiosError(error)) {
        const { response } = error;
      ...
      <Content>
        ...
        <Modal isOpen={modal.add} toggle={() => setModal({ add: false })} className="modal-dialog-centered" size="lg">
          <ModalBody>
            ...
            <div className="p-2">
              <h5 className="title">Add Title</h5>
              <div className="mt-4">
