...
const UserRoles = () => {
  ...
  const onFormSubmit = async (submitData) => {
    ...
    try {
      const RoleRes = await instanceAxios.post("Roles", {
        name: name,
      });
  ...
  const onEditSubmit = async (submitData) => {
    ...
    try {
      const RoleRes = await instanceAxios.put("Roles", {
        id: submitData.id,
        name: name,
      });
  ...
  const suspendUser = async (id) => {
    try {
      const { data } = await instanceAxios.delete(`Roles/${id}`);
      ...
      <Content>
        ...
        <Modal isOpen={modal.edit} toggle={() => setModal({ edit: false })} className="modal-dialog-centered" size="lg">
          <ModalBody>
            ...
            <div className="p-2">
              <h5 className="title">Update Role</h5>
              <div className="mt-4">
